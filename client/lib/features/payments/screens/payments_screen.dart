...
class _PaymentsScreenState extends State<PaymentsScreen>
  ...
  Widget _buildBentoStats(bool isDesktop) {
    return FutureBuilder<PaymentSummary>(
      ...
      builder: (context, snap) {
        final summary = snap.data;
        final recoveryRate = summary != null && (summary.totalRevenue + summary.pendingRevenue) > 0
            ? summary.totalRevenue / (summary.totalRevenue + summary.pendingRevenue)
            : 0.0;

        if (isDesktop) {
          return Row(children: [
            Expanded(flex: 2, child: _RecoveryCard(rate: recoveryRate, loading: !snap.hasData)),
    ...
    return FutureBuilder<PaymentsPage>(
      ...
      builder: (context, snap) {
        ...
        final payments = allPayments.where((p) => 
          p.memberName.toLowerCase().contains(_searchQuery.toLowerCase())).toList();
            ...
            if (isDesktop)
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(flex: 2, child: _ColHeader('ACTION', align: TextAlign.right)),
                ]),
              ),
            ...
            if (snap.data != null && snap.data!.pages > 1)
              Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('PAGE ${snap.data!.page} OF ${snap.data!.pages}', style: GoogleFonts.spaceGrotesk(fontSize: 9, color: AppColors.onSurfaceVariant)),
                    Row(children: [
                      IconButton(onPressed: _currentPage > 1 ? () => _setPage(_currentPage - 1) : null, icon: const Icon(Icons.chevron_left)),
                      IconButton(onPressed: _currentPage < snap.data!.pages ? () => _setPage(_currentPage + 1) : null, icon: const Icon(Icons.chevron_right)),
                    ]),
                  ],
                ),
              ),
          ...
          child: isDesktop
              ? Row(children: [
                  _FooterStat(
                    ...
                    value: summary != null ? 'Rs.${summary.totalRevenue.toStringAsFixed(0)}' : '—',
                  ),
                  Container(
                    width: 1, height: 48,
              ...
              : Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(child: _FooterBtn(label: 'DOWNLOAD', isPrimary: false)),
                      const SizedBox(width: 12),
                      Expanded(child: _FooterBtn(label: 'SEND REMINDERS', isPrimary: true)),
                    ]),
                  ],
                ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    if (!isDesktop) {
      return Container(
        ...
        child: Row(children: [
          AvatarPlaceholder(initials: payment.initials, size: 40, paymentStatus: payment.status),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(payment.memberName.toUpperCase(),
                  style: GoogleFonts.lexend(
          ...
          isPaid
              ? GestureDetector(
                  ...
                  child: const Icon(Icons.check_circle, color: AppColors.primaryContainer, size: 20),
                )
              : GestureDetector(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(children: [
              ...
              Expanded(flex: 2, child: Text('Rs.${payment.amount.toStringAsFixed(2)}',
                style: GoogleFonts.spaceGrotesk(
                  fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.onSurface,
                ))),
              ...
              Expanded(
                ...
                child: Align(
                  ...
                  child: isPaid
                      ? IconButton(
                          onPressed: () => _showRevertConfirm(context, onUnmarkPaid),
                          ...
                          child: Container(
                            ...
                            child: Text('RECEIVED',
                              style: GoogleFonts.lexend(
                                ...
                                color: AppColors.onPrimaryContainer, letterSpacing: 1.5,
                              )),
                          ),
                        ),
                ),
  ...
  void _showRevertConfirm(BuildContext context, VoidCallback onConfirm) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('REVERT PAYMENT',
          style: GoogleFonts.lexend(fontWeight: FontWeight.w900, color: AppColors.onSurface)),
        content: Text('Are you sure you want to mark this payment as unpaid?',
          style: GoogleFonts.manrope(color: AppColors.onSurfaceVariant)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: Text('CANCEL', style: GoogleFonts.spaceGrotesk(color: AppColors.onSurfaceVariant, fontWeight: FontWeight.w700)),
          ),
          TextButton(
            onPressed: () {
            ...
            child: Text('REVERT', style: GoogleFonts.spaceGrotesk(color: AppColors.error, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    switch (status) {
      case PaymentStatus.overdue:
        bg = AppColors.errorContainer; textColor = AppColors.onErrorContainer; label = 'OVERDUE';
        break;
      case PaymentStatus.pending:
        bg = AppColors.surfaceContainerHighest; textColor = AppColors.onSurfaceVariant; label = 'PENDING';
        break;
      case PaymentStatus.paid:
        bg = AppColors.primaryContainer.withOpacity(0.15);
        textColor = AppColors.primaryContainer; label = 'PAID';
        break;
    }
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(label,
          style: GoogleFonts.spaceGrotesk(
            fontSize: 9, fontWeight: FontWeight.w700, color: textColor, letterSpacing: 1.5,
          )),
      ]),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Icon(Icons.warning_outlined, color: AppColors.onSecondaryContainer, size: 28),
              const Icon(Icons.arrow_forward, color: AppColors.onSecondaryContainer, size: 18),
            ],
          ),
          const SizedBox(height: 16),
          Text(count.toString().padLeft(2, '0'),
            style: GoogleFonts.lexend(
              ...
              color: AppColors.onSecondaryContainer, letterSpacing: -1,
            )),
          Text('OVERDUE PAYMENTS',
            style: GoogleFonts.spaceGrotesk(
              fontSize: 9, color: AppColors.onSecondaryContainer.withOpacity(0.8),
      ...
      children: [
        ...
        Text(value,
          style: GoogleFonts.lexend(
            ...
            color: valueColor ?? AppColors.onSurface, letterSpacing: -1,
          )),
      ],
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isPrimary ? null : Border.all(color: AppColors.outlineVariant.withOpacity(0.3)),
          boxShadow: isPrimary ? [BoxShadow(
            color: AppColors.secondary.withOpacity(0.2), blurRadius: 30,
          )] : null,
        ),
        child: Text(label,
