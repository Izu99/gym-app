                    ...
                    text: TextSpan(children: [
                      TextSpan(text: 'COMMAND ',
                        style: GoogleFonts.lexend(
                          fontSize: isDesktop ? 64 : 36, fontWeight: FontWeight.w900,
                          color: AppColors.onSurface, letterSpacing: -2, height: 0.95,
                        )),
                      TextSpan(text: 'CENTER',
                        style: GoogleFonts.lexend(
                          fontSize: isDesktop ? 64 : 36, fontWeight: FontWeight.w900,
                          color: AppColors.onSurfaceVariant, letterSpacing: -2, height: 0.95,
                        )),
                    ]),
                  ),
                  const SizedBox(height: 12),
                  Text('REAL-TIME FACILITY OUTPUT & TELEMETRY',
      ...
      builder: (context, snap) {
        ...
        if (isDesktop) {
          return Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(flex: 2, child: _AttendanceCard(
                count: stats?.dailyAttendance, delta: stats?.attendanceDelta, loading: loading,
              )),
              const SizedBox(width: 24),
              Expanded(child: _RevenueCard(
                revenue: stats?.monthlyRevenue, loading: loading,
              )),
              const SizedBox(width: 24),
              Expanded(child: _ActiveMembersCard(
                active: stats?.activeMembers, newThisMonth: stats?.newThisMonth, loading: loading,
              )),
            ],
          );
        }
        return Column(children: [
          _AttendanceCard(count: stats?.dailyAttendance, delta: stats?.attendanceDelta, loading: loading),
          const SizedBox(height: 16),
          Row(children: [
            Expanded(child: _RevenueCard(revenue: stats?.monthlyRevenue, loading: loading)),
    ...
    if (isDesktop) {
      return Row(
        ...
        children: [
          ...
          const SizedBox(width: 24),
          Expanded(flex: 2, child: _AttendanceLineChart(future: _attendanceFuture)),
        ],
      );
    }
    return Column(children: [
  ...
  Widget _buildMembershipBreakdown(bool isDesktop) {
    if (isDesktop) {
      return Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(flex: 2, child: _MemberTierTable(breakdownFuture: _tierFuture, tiersFuture: _tiersListFuture)),
          const SizedBox(width: 24),
    ...
    return Column(children: [
      ...
      const SizedBox(height: 16),
      _MemberTierTable(breakdownFuture: _tierFuture, tiersFuture: _tiersListFuture),
    ]);
  }
  ...
  Widget _buildBottomSection(bool isDesktop) {
    if (isDesktop) {
      return Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(flex: 2, child: _LiveCheckinsSection(future: _checkinsFuture)),
          const SizedBox(width: 48),
          Expanded(child: FutureBuilder<DashboardStats>(
            future: _statsFuture,
  ...
  Widget build(BuildContext context) {
    final deltaStr = delta == null
        ...
        : delta! >= 0
            ...
            : '${delta!.toStringAsFixed(1)}% vs yest.';
    final deltaColor = (delta ?? 0) >= 0 ? AppColors.primaryContainer : AppColors.secondary;
              ...
              : Row(
                  ...
                  children: [
                    ...
                    if (deltaStr.isNotEmpty)
                      Text(deltaStr,
                        style: GoogleFonts.spaceGrotesk(
                          fontSize: 11, color: deltaColor, fontWeight: FontWeight.w700,
                        )),
                  ],
                ),
...
class _RevenueCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = revenue != null && projected != null ? revenue! / projected : 0.0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          loading
              ? _shimmer(100, 36)
              : Text(revenue != null ? _fmtCurrency(revenue!) : '—',
                  style: GoogleFonts.lexend(
          ...
          loading
              ...
              : Text(projected != null ? 'Projected: ${_fmtCurrency(projected)}' : '',
                  style: GoogleFonts.spaceGrotesk(
...
class _ActiveMembersCard extends StatelessWidget {
  ...
  final bool loading;
  const _ActiveMembersCard({this.active, this.newThisMonth, required this.loading});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          loading
              ? const SizedBox(height: 4)
              : Text(newThisMonth != null ? '+$newThisMonth new this month' : '',
                  style: GoogleFonts.spaceGrotesk(
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<DailyRevenue>>(
      ...
      builder: (context, snap) {
        ...
        final maxY = data.isEmpty ? 10000.0
                ...
                children: [
                  ...
                  if (data.isNotEmpty)
                    Container(
                      ...
                      child: Text('${data.length} DAYS',
                        style: GoogleFonts.spaceGrotesk(
                          fontSize: 9, color: AppColors.primaryContainer,
                        ...
                        : BarChart(BarChartData(
                                ...
                                sideTitles: SideTitles(
                                  ...
                                  getTitlesWidget: (v, _) {
                                    ...
                                    final label = data[i].date.length >= 2
                                        ? data[i].date.substring(data[i].date.length - 2)
                                        : data[i].date;
                                    return Padding(
                            ...
                            gridData: FlGridData(
                              ...
                              getDrawingHorizontalLine: (_) => FlLine(
                                color: AppColors.outlineVariant.withOpacity(0.1), strokeWidth: 1,
                              ),
                            ),
                            borderData: FlBorderData(show: false),
                            barGroups: data.asMap().entries.map((e) {
                              final isMax = e.value.revenue ==
    ...
    return FutureBuilder<List<AttendanceStat>>(
      ...
      builder: (context, snap) {
        ...
        final spots = data.asMap().entries
            ...
            .toList();
        final maxY = data.isEmpty ? 160.0
            : (data.map((d) => d.count.toDouble()).reduce((a, b) => a > b ? a : b) * 1.2);
        final minY = data.isEmpty ? 0.0
            : (data.map((d) => d.count.toDouble()).reduce((a, b) => a < b ? a : b) * 0.8);
                            ...
                            lineBarsData: [LineChartBarData(
                              ...
                              dotData: FlDotData(
                                ...
                                getDotPainter: (_, __, ___, ____) => FlDotCirclePainter(
                                  radius: 3, color: AppColors.primaryContainer, strokeWidth: 0,
                                ),
                              ),
              ...
              ...data.asMap().entries.map((e) {
                ...
                return Padding(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Text(t.tier.toUpperCase(),
                      style: GoogleFonts.spaceGrotesk(
                        fontSize: 9, color: AppColors.onSurfaceVariant, letterSpacing: 1.5,
                      ))),
                    Text('${t.count} (${pct.toStringAsFixed(0)}%)',
                      style: GoogleFonts.lexend(
...
class _MemberTierTable extends StatelessWidget {
  ...
  const _MemberTierTable({required this.breakdownFuture, required this.tiersFuture});
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text('MEMBERSHIP BREAKDOWN',
                  style: GoogleFonts.spaceGrotesk(
                    fontSize: 9, color: AppColors.onSurfaceVariant,
                    letterSpacing: 2.5, fontWeight: FontWeight.w700,
                  )),
              ...
              if (snap.connectionState == ConnectionState.waiting)
                ...
                ...() {
                  ...
                  return breakdown.map((r) {
                    ...
                    final tierInfo = tiers.firstWhere((t) => t.id == r.tierId || t.name == r.tier, 
                      orElse: () => Tier(id: r.tierId, name: r.tier, monthlyFee: 0));
                    ...
                    return Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(flex: 2, child: Text('${r.count}',
                          style: GoogleFonts.spaceGrotesk(
                            fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.onSurfaceVariant,
                          ))),
                        Expanded(flex: 2, child: Text(_fmtCurrency(revenue.toDouble()),
                          style: GoogleFonts.spaceGrotesk(
...
class _LiveCheckinsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<ApiAttendanceRecord>>(
      ...
      builder: (context, snap) {
        ...
        final present = records.where((r) => r.status == AttendanceStatus.present).toList();
        ...
        return Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            if (present.isEmpty && snap.connectionState != ConnectionState.waiting)
              Container(
              ...
              ...present.take(5).toList().asMap().entries.map((e) {
                ...
                return _CheckinItem(
                  name: r.memberName,
                  tier: r.tierLabel,
                  time: r.checkinTime != null ? _formatTime(r.checkinTime!) : 'TODAY',
                  isLive: isFirst,
                  bgColor: e.key % 2 == 0
  ...
  static String _formatTime(String iso) {
    try {
      ...
      final h = d.hour % 12 == 0 ? 12 : d.hour % 12;      final m = d.minute.toString().padLeft(2, '0');
...
class _CheckinItem extends StatelessWidget {
  ...
  final Color bgColor;
  const _CheckinItem({
    required this.name, required this.tier,
    required this.time, required this.isLive, required this.bgColor,
  });
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          ...
          child: const Icon(Icons.person_outline, color: AppColors.onSurfaceVariant, size: 20),
        ),
        const SizedBox(width: 16),
        Expanded(
          child: Column(
      ...
      children: [
        ...
        if (overdueMembers > 0)
          _UrgentItem(
            ...
            tag: 'MEMBERS AT RISK',
            title: '$overdueMembers MEMBER${overdueMembers != 1 ? 'S' : ''} OVERDUE',
          ),
...
class _UrgentItem extends StatelessWidget {
  ...
  const _UrgentItem({required this.color, required this.tag, required this.title});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(tag,
            style: GoogleFonts.spaceGrotesk(
              fontSize: 9, color: color, letterSpacing: 2, fontWeight: FontWeight.w700,
            )),
          const SizedBox(height: 4),
          Text(title,
            style: GoogleFonts.lexend(
